...
from load_with_colmap import DataLoaderWithCOLMAP
...
from models.intrinsics import FocalModel
from models.poses import PoseModel
...
def parse_args():
    ...
    parser.add_argument('--eval_img_interval', default=20, type=int, help='eval images every this epoch number')
    ...
    parser.add_argument('--train_skip', type=int, default=6, help='skip every this number of imgs')
...
def train(args):
    my_devices = torch.device('cuda:' + str(0))
    
    print('Creating folders')
    exp_root_dir = Path(os.path.join('./logs/mnerf', args.scene_name))
    exp_root_dir.mkdir(parents=True, exist_ok=True)
    experiment_dir = Path(os.path.join(exp_root_dir, gen_detail_name(args)))
    experiment_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy('./run_nerf.py', experiment_dir)

    store_pose_history = True
    if store_pose_history:
    ...
    model = Nerf(pos_enc_in_dims, dir_enc_in_dims, args.hidden_dims)
    model = model.to(device=my_devices)

    print("Loading focal parameter model")
    if args.start_refine_focal_epoch > -1:
        focal_net = FocalModel(scene_train.H, scene_train.W, args.learn_focal, args.fx_only,
                               order=args.focal_order, init_focal=scene_train.focal)
    else:
        focal_net = FocalModel(scene_train.H, scene_train.W, args.learn_focal, args.fx_only, order=args.focal_order)
    focal_net = focal_net.to(device=my_devices)

    print("Loading pose model")
    if args.start_refine_pose_epoch > -1:
        pose_param_net = PoseModel(scene_train.N_imgs, args.learn_R, args.learn_t, scene_train.c2ws)
    else:
        pose_param_net = PoseModel(scene_train.N_imgs, args.learn_R, args.learn_t, None)
    pose_param_net = pose_param_net.to(device=my_devices)

    print("Setting up and optimiser")
